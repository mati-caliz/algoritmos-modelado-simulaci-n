...
def simplify_vector(vector):
    """
    Simplifica un vector propio normalizándolo al componente de mayor magnitud.

    Parámetros:
    - vector: Vector propio a simplificar.

    Retorna:
    - vector: Vector propio simplificado.
    """
...
def print_general_equation(eigenvalues, eigenvectors):
    """
    Genera y muestra la ecuación general del sistema lineal.
    ...
    for i, eigenvalue in enumerate(eigenvalues):
        ...
        print(f"Ecuación {i + 1}: x(t) = {x_eq}, y(t) = {y_eq}")
...
def main():
    ...
    V1 = simplify_vector(eigenvectors[:, 0])
    V2 = simplify_vector(eigenvectors[:, 1])

    # Mostrar valores propios y vectores propios simplificados
    print("Valores propios:")
    for i, eigenvalue in enumerate(eigenvalues):
        print(f"\nλ{i + 1} = {eigenvalue.real}")
        vector = simplify_vector(eigenvectors[:, i])
